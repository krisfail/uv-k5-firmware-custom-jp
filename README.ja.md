...
- `W/N`: `WIDE`、`WIDE+`、`NARROW`の3段階の受信帯域
...
メニューの`RXExt`を`OFF`にすると、プリセット、WIDE+、受信モードの`SINGLE`、メモリーバンク絞り込み、`AUTO`スケルチ、AGCガード、一時スキップを停止します。受信専用・PTTモニター、日本語表示、FM放送の`76.0–95.0 MHz`制限は変わりません。旧形式の保存データは互換性のためONとして扱います。
