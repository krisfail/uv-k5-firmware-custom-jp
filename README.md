...
- `W/N`は`WIDE`、`WIDE+`、`NARROW`の3段階です。`WIDE+`は、弱い信号でも受信帯域を狭めず、WIDEの帯域を維持します。`SetNFM`が有効なときは、`NARROW`をさらに狭いFMフィルターへ切り替えます。
- `受信モード`は`MAIN ONLY`、`DUAL RX`、`SINGLE`から選べます。`SINGLE`ではVFO Aを固定して受信し、A/B切替を無効にします。`MAIN ONLY`または`DUAL RX`を選ぶと通常の2系統へ戻ります。
- `Bank`は`ALL`または`B1`～`B8`を選び、メモリースキャンとチャンネル選択の対象を絞ります。`BnkSet`で現在のメモリーチャンネルを1つのバンクへ割り当てられます。`NONE`は未所属です。バンク所属は既存のチャンネル属性を保持したまま保存されます。
- `スケルチ`の`AUTO`は、現在のFM受信周波数を再構成するときにRSSI・ノイズ・グリッチを短時間測定し、その受信状態へ閾値を適用します。工場校正値やEEPROMの校正表は書き換えません。信号受信中の連続追従ではありません。
- 通常FMでは、RSSIまたは受信ゲインの急変を検出したときにAGCを短時間固定し、急激な音量変化を抑えます。AM補正、FM放送、スキャン中はこの処理の対象外です。
...
| `受信モード` | MAIN ONLY・DUAL RX・SINGLEの受信動作 |
| `W/N` | WIDE・WIDE+・NARROWの受信帯域 |
| `Bank` / `BnkSet` | 使用するバンクの選択／現在のメモリーチャンネルの所属設定 |
| `スケルチ` | 無信号時に音声を抑えるレベル。`AUTO`は受信環境から一度調整 |
