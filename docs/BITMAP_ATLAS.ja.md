...
`gFontBig`は14 bytes/glyph（7列×2 OLEDページ）として認識し、連続した長いbyte列ではなくglyphごとのグリッドに配置します。`gFontBigDigits`も同じ2ページ形式として表示します。`gFontJapaneseExtraLarge`は20 bytes/glyph（10列×2 OLEDページ）の特大字形として表示します。K5の容量を守るため、特大字形はコード注釈付きの4スロットだけを持つ疎な配列です。`gFontBig`のglyphラベルは`0x21`（`!`）からのコード値です。`#if 0`で無効化された旧配列は除外します。
...
特大字形は同じ16行契約で横幅だけを10列へ拡張しています。`UI_PrintStringJapaneseExtraLarge`は、特大表へ登録された日本語コードだけを指定位置へ描画する専用APIです。現在の4スロットは準備用の空字形で、実際の画面からはまだ呼び出しません。字形を作成した後、用途ごとに通常大字形と使い分けます。
