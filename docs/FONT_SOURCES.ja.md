...
`tools/import_public_domain_bitmap_font.py`は、BDFのUnicode/JIS対応を読み取り、ソースリポジトリの既存字形や`reverse-engineering-handoff`の抽出物を参照せず、次の規則だけで出力します。
...
`reverse-engineering-handoff`内の抽出物は、実装字形の入力には使用していません。パブリックドメインの公開BDFを入力にした独立変換として、出所と生成規則を分離しています。
